...
def read_command_line():
    ...
    parser.add_argument("-e", "--tournament-number", required=False, default="1", help="tournament number")
    parser.add_argument("-n", "--number-of-rounds", type=int, default=0, help="Number of rounds, overrides file value")
    parser.add_argument(
        "-p",
        "--pre-determined",
        required=False,
    ...
    parser.add_argument("-s", "--swiss", required=False, action="store_true", help="Use rules for swiss tournament")
    parser.add_argument(
        "-g",
        "--game-score",
        required=False,
...
def write_output_file(params, chessfile, tb):
    ...
    if params["check"] and tb is not None:
