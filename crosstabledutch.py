...
class crosstable_dutch(crosstable):
    """
    ...
    def update_edge(self, edge):
        ...
        coltrans = {"nc": "n", "w0": "w", "w1": "W", "w2": "W", "b0": "b", "b1": "B", "b2": "B"}
        maxpsd = self.maxpsd
        ...
        if c["canmeet"]:
            ...
            if a["scorelevel"] >= scorelevel and b["scorelevel"] == scorelevel:
                ...
                if a["cop"] != "nc" and a["cop"][0].lower() == b["cop"][0].lower():
                    q[QC12] = 1
                    cweight += weight[QC12]
                ...
                c15 = 1 if (a["acc"] < b["acc"]) and (a["flt"] & flt.UF1.value) or (a["acc"] > b["acc"]) and (b["flt"] & flt.UF1.value) else 0
                if c15:
                    q[QC15] = 1
                    cweight += weight[QC15]

                # c17 minimize the number of players who receive upfloft two rounds before
                c17 = 1 if (a["acc"] < b["acc"]) and (a["flt"] & flt.UF2.value) or (a["acc"] > b["acc"]) and (b["flt"] & flt.UF2.value) else 0
                if c17:
                    q[QC17] = 1
                    cweight += weight[QC17]
    ...
    def init_cweights(self, scorelevel, nodes):
        ...
        depth[QC12] = cc
        depth[QC13] = cc
        depth[QC14] = len([node for node in nodes if node["scorelevel"] >= scorelevel and (node["flt"] & flt.DF1.value)]) +1
        depth[QC15] = len([node for node in nodes if node["scorelevel"] <= scorelevel and (node["flt"] & flt.UF1.value)]) +1
        depth[QC16] = len([node for node in nodes if node["scorelevel"] >= scorelevel and (node["flt"] & flt.DF2.value)]) +1
        depth[QC17] = len([node for node in nodes if node["scorelevel"] <= scorelevel and (node["flt"] & flt.UF2.value)]) +1
